...
if USE_SUPABASE_STORAGE:
    ...
    # NB: S3 API живёт на поддомене *.storage.supabase.co (для PUT/HEAD/DELETE — с подписью).
    # Публичные object-URL — на *.supabase.co/storage/v1/object/public/<bucket>/<key> (без подписи).
    # AWS_S3_CUSTOM_DOMAIN заставляет storage.url() возвращать публичный URL,
    # а не S3 API path (который без подписи отдаёт 403).
